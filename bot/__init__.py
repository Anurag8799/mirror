...
try:
    ...
    log_warning("CHANNEL_USERNAME not provided! Using default @MOVIES_ZILAA")
    CHANNEL_USERNAME = "@MOVIES_ZILAA"
try:
    FSUB_CHANNEL_ID = int(getConfig("FSUB_CHANNEL_ID"))
except KeyError:
    log_warning("CHANNEL_USERNAME not provided! Using default id of @MOVIES_ZILAA")
...
try:
    ...
    if len(AUTHOR_URL) == 0:
        AUTHOR_URL = 'https://t.me/Am_RoBots'
except KeyError:
    AUTHOR_URL = 'https://t.me/Am_RoBots'
try:
    GD_INFO = getConfig('GD_INFO')
    if len(GD_INFO) == 0:
        GD_INFO = 'Uploaded by Anurag Mirror Bot'
except KeyError:
    GD_INFO = 'Uploaded by Anurag Mirror Bot'
try:
    TITLE_NAME = getConfig('TITLE_NAME')
    if len(TITLE_NAME) == 0:
        TITLE_NAME = 'Anurag-Mirror-Search'
except KeyError:
    TITLE_NAME = 'Anurag-Mirror-Search'
try:
